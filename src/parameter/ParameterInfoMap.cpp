...
)
{
    ...
    std::sort(params.begin(), params.end(),
        [&](const std::shared_ptr<ParameterInfo>& a, const std::shared_ptr<ParameterInfo>& b) {
            if (inAlphabeticalOrder)
            {
...
ParameterInfoMap::ParameterInfoMap(const std::vector<std::shared_ptr<ParameterInfo>>& camParams)
{
...
void ParameterInfoMap::addParameter(const std::shared_ptr<ParameterInfo>& param)
{
    categoryMap_[param->category()][param->visibilityLevel()].push_back(param);
}

std::vector<std::shared_ptr<ParameterInfo>> ParameterInfoMap::getFilteredParams(
    const std::optional<std::string>& category,
    std::optional<VisibilityLevel> visibility,
    bool inAlphabeticalOrder
) const
{
    std::vector<std::shared_ptr<ParameterInfo>> result;
...
std::shared_ptr<ParameterInfo> ParameterInfoMap::getParameter(
    ...
    for (const auto& [category, visibilityMap] : categoryMap_)
    {
        for (const auto& [level, params] : visibilityMap)
        {
            auto it = std::find_if(params.begin(), params.end(),
                [&paramName](const std::shared_ptr<ParameterInfo>& param)
                {
            ...
            if (it != params.end())
            {
                return *it;  // Return found parameter wrapped in optional
            }
        }
    }
...
std::vector<std::string> ParameterInfoMap::getAllCategories() const
{
...
std::vector<VisibilityLevel> ParameterInfoMap::getAllVisibilityLevels() const
{
